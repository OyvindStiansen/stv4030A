...
Under, we illustrate how to read a selection of file formats into R. The data are gathered from the `textreadr` package, and includes all three debates of the presidential debates between Romney and Obama in 2012. There are 4 variables in the data:
...
We can load the data into R intitially by using the `data()` function:

```{r}
#| label: load_cases_data
#| message: false
#| warning: false
library(tidyverse)
...
pres_deb <- presidential_debates_2012 # just making a shorter name for the object
rm(presidential_debates_2012)

pres_deb %>% 
  mutate(dialogue = str_sub(dialogue, 1, 30)) %>% 
  tail()
...
#| label: save_rda
#| eval: false
save(pres_deb, file = "../data/pres_deb.rda")
...
#| label: rda_load
#| eval: false
load("../data/pres_deb.rda")
```

A potentially confusing detail with these functions is that the file name (in our case "cases.rda") does not necessarily match the name of the object in the *Environment*. The objects will always keep its *Environment* name when loaded from an RData file.
...
pres_deb <- read_csv("../data/pres_deb.csv", show_col_types = FALSE)
...
#| eval: false
library(haven)
cases <- read_sav("../data/pres_deb.sav")
```
...
#| label: dta_files
#| eval: false
pres_deb <- read_dta("../data/pres_deb.dta")
```
...
Raw textfiles is also a common format for working with text data. The format does not have any overhead, which makes the files relatively small in size and they are flexible to work with. A common way to structure `.txt` files is to have each file be a document with a file name that indicates which document we are looking at. For example, we can store the titles in our `pres_deb` dataset into text files with their `id` as file names (here only with the first 10 rows of the data):

```{r}
#| label: save_txt
#| eval: true
#| output: false
lapply(1:10, function(x) writeLines(pres_deb$dialogue[x], 
                                    paste0("../data/dialogue_txt/", 
                                           pres_deb$tot[x], 
                                           pres_deb$person[x], 
                                           ".txt")))
...
dialogue_files <- list.files("../data/dialogue_txt", 
                             full.names = TRUE, 
                             pattern = ".txt")
dialogue_files
...
dialogues <- lapply(dialogue_files, readLines)

class(dialogues) # lapply always returns a list

dialogues[[1]] # This is the first title

```

We can now put the texts into a data frame by using `unlist()` on the `dialogues` object:

```{r}
#| label: txt_dataframe

dialogue_txt <- data.frame(id = dialogue_files, 
                        text = unlist(dialogues))

# Trying to (sort of) reengineer the original data
dialogue_txt %>% 
  mutate(person = str_extract(id, "[A-Z]+"),
         tot = str_extract(id, "([0-9]+\\.*)+"),
         text = str_sub(text, 1, 50)) %>% 
  select(person, tot, text)
```
...
names(dialogues) <- str_extract(dialogue_files, "([0-9]+\\.*)+[A-Z]+")
names(dialogues)
...
dialogue_txt <- data.frame(text = unlist(dialogues),
                           id = names(dialogues))

glimpse(dialogue_txt)
```
...
object.size(dialogue_txt) - object.size(dialogues)
```
